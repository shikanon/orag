...
`evaluation_runs.metrics` 是 JSON 指标快照，当前会保存 `answer_accuracy`、`accuracy`、`hit_rate`、`citation_hit_rate` 以及聚合后的 `context_recall`、`citation_precision`、`latency_p95_ms`、`cache_hit_rate`。`GET /v1/evaluations/{id}` 查询的是运行级汇总，不返回逐样本 `evaluation_results` 明细。历史已存运行可能没有新增的 `answer_accuracy` 和 `citation_hit_rate` 键。

`evaluation_results.metrics` 保存逐样本指标，包括 `answer_accuracy`、`accuracy`、`citation_hit_rate`、`context_recall`、`citation_precision`、`latency_ms` 和 `cache_hit`。这些明细用于后续分析和扩展，目前公开 API 主要暴露运行级汇总。
...
`POST /v1/optimizations` 对候选 `profiles` 和 `top_ks` 做确定性网格枚举。当前优化目标是运行级 `answer_accuracy`，也就是上述答案命中率。
...
4. 候选结果记录 `profile`、`top_k`、`score` 和 `run_id`，其中 `score` 优先取 `run.Metrics["answer_accuracy"]`，仅在历史运行缺少该指标时回退到 `run.Accuracy`。
...
- 分数依赖当前 rule-based `answer_accuracy`，不能等价为真实业务满意度或完整答案质量。
