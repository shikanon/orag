...
| 学习工程模块效果 | 控制台 `/tutorials` -> [`tutorials/clone-and-pack-install.md`](./tutorials/clone-and-pack-install.md) -> [`tutorials/p1-structured-json-candidate.md`](./tutorials/p1-structured-json-candidate.md) -> [`tutorials/p2-recursive-chunk-candidate.md`](./tutorials/p2-recursive-chunk-candidate.md) | 克隆不可变模板并安装经服务器校验的 Pack；声明 P1/P2 的文本 Quick Pack 可在冻结输入下对比 P0 的真实评测指标与实际索引事实，P3–P8、Replay 和视觉/视频运行仍明确标为未开放。 |
