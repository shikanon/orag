...
最新进展：Pack 声明的 P2 `p2_recursive_400_80` 已成为 P0 的直接子实验。教程 P0/P1 分块固定为 800/120，P2 仅使用 Basic parser 的 400/80 递归分块和独立知识库；运行持久化实际 chunk 数与平均文本单元，比较将这些 `index_metrics` 与普通评测指标严格分开。受控 `1.0.2` fixture 与真实 PostgreSQL/Qdrant/浏览器 E2E 证明 P0/P1/P2 的隔离与可测索引形状；官方公开 `1.0.2` Pack 仍需独立匿名 HTTPS、MIME、长度和 SHA-256 发布流水线。
...
- 已完成官方教程的 clone 与 Pack 安装、声明运行时的 `text-rag` Quick Pack 固定 P0 基线，以及 Pack 声明的 P1 结构化 JSON 与 P2 400/80 递归分块候选（冻结输入、独立索引、真实评测指标和测量到的索引事实对比）。待完成 P3–P8 候选/Benchmark Run、官方 Replay 和视觉/视频执行；默认公共 OSS 可用性及官方 `1.0.1`/`1.0.2` Pack 发布仍是外部前置条件。
