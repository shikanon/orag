...
- Migration status, Qdrant collection compatibility checks, disaster-recovery documentation, a backup-artifact verifier, and a complete isolated PostgreSQL + Qdrant backup/restore drill with post-restore cited-query and trace verification are available; retain real backup/restore drill evidence from independent reference deployments next.
...
- Threat-model and test secret injection/rotation, log redaction, prompt/document recording, and cross-tenant queries. A reviewable [threat model](./docs/security/threat-model.md) and a no-real-key isolated API-key immediate-cutover drill are now available; the `ORAG_ENV=production` startup guard rejects demo/weak credentials, debug/mock execution, mock storage, and non-HTTPS local URLs. Server-side JWT/pepper rotation evidence from independent reference deployments and an external threat-model review remain outstanding.
