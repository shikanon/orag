...
test.describe('real PostgreSQL + Qdrant tutorial Pack installation', () => {
  ...
  test('creates a project and installs the server-verified Quick Pack', async ({ page }) => {
    ...
    await page.getByRole('button', { name: '运行 P3 上下文化候选' }).click()
    ...
    await expect(page.getByText('contextualized_chunk_count')).toBeVisible({ timeout: 60_000 })
    await expect(page.getByText('average_context_tokens')).toBeVisible()
