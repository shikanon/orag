...
| 学习工程模块效果 | 控制台 `/tutorials` -> [`tutorials/clone-and-pack-install.md`](./tutorials/clone-and-pack-install.md) -> [`tutorials/text-rag-benchmark-run.md`](./tutorials/text-rag-benchmark-run.md) -> [`tutorials/text-rag-official-replay.md`](./tutorials/text-rag-official-replay.md) | `text-rag` Quick/Benchmark 可在冻结输入下运行 P0–P8；`video-rag` 支持所有者授权的私有导入、时序证据与 temporal P0。官方 text-rag Replay 是固定环境的只读快照；视觉 Live Run/Replay 与视频公开 Replay 尚未发布。 |
