...
- 已完成官方教程的 clone 与 Pack 安装、声明运行时的 `text-rag` Quick Pack 固定 P0 基线，以及 Pack 声明的 P1 结构化 JSON、P2 400/80 递归分块、P3 严格失败上下文化、P4 P0 索引复用的 pure sparse 检索、P5 P0 索引复用的三路 multi-query 和 P6 P0 索引复用的 rerank 候选（冻结输入、真实评测指标和持久化审计事实对比）。待完成 P7–P8 候选/Benchmark Run、官方 Replay 和视觉/视频执行；默认公共 OSS 可用性及官方 `1.0.1`–`1.0.6` Pack 发布仍是外部前置条件。
