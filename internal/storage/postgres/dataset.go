...
func (r *Repository) AddDatasetItem(ctx context.Context, item dataset.Item) (dataset.Item, error) {
	...
	_, err = r.Pool.Exec(ctx, `
		...
		VALUES($1,$2,$3,$4,$5,$6)`,
		item.ID, item.DatasetID, item.Query, item.GroundTruth, relevantBody, diversityBody)
	return item, err
}

func (r *Repository) DatasetItems(ctx context.Context, datasetID string) ([]dataset.Item, error) {
	rows, err := r.Pool.Query(ctx, `
		SELECT id, dataset_id, query, ground_truth, relevant_doc_ids, diversity_annotations
		FROM dataset_items
		WHERE dataset_id=$1
