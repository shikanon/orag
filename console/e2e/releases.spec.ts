...
test('creates a legacy candidate and submits a promotion request', async ({ page }) => {
  await authenticateConsole(page)
  await page.route('**/v1/projects', (route) => route.fulfill({ json: { projects: [{ id: 'prj_a', tenant_id: 'tenant_a', name: 'Release E2E', description: '', created_at: '', updated_at: '' }] } }))
  await page.route('**/v1/projects/prj_a', (route) => route.fulfill({ json: { id: 'prj_a', tenant_id: 'tenant_a', name: 'Release E2E', description: '', created_at: '', updated_at: '' } }))
  ...
  await page.getByRole('button', { name: '创建旧版记录' }).click()
