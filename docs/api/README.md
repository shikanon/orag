...
对带受支持运行时声明的 `text-rag` Quick/Benchmark Pack，`POST /v1/projects/{project_id}/tutorial-experiments/{experiment_id}/runs` 接收 `variant` 与幂等键；未传 `variant` 等价于 P0 `baseline`。服务端从保存的校验后 Manifest 推导私有对象、知识库、数据集和固定 profile/Top-K，随后创建普通评测运行。P0–P8 仅在输入 fingerprint 相同的完成 P0 之后启动；`GET .../{run_id}/comparison` 只在持久化证据可证明可比性时返回真实评测指标和 `index_metrics`。`GET /v1/tutorials/text-rag/replay` 是独立的固定环境、只读官方快照，不会运行模型、创建项目或返回存储坐标；视觉/视频仍返回 Replay 未发布。完整边界见 [`../tutorials/clone-and-pack-install.md`](../tutorials/clone-and-pack-install.md)、[`../tutorials/text-rag-benchmark-run.md`](../tutorials/text-rag-benchmark-run.md) 和 [`../tutorials/text-rag-official-replay.md`](../tutorials/text-rag-official-replay.md)。
