...
# 1.0.0 Pack. For this controlled browser test, serve the immutable 1.0.5 P5
# multi-query fixture through the 1.0.0 catalog path after rewriting only the
...
cp -R "$root/tests/fixtures/tutorial-packs/text-rag/1.0.5/quick" "$public_catalog/text-rag/1.0.0/quick"
sed -i.bak 's/"version": "1.0.5"/"version": "1.0.0"/' "$public_catalog/text-rag/1.0.0/quick/manifest.json"
