...
| 现象 | 优先检查 | 处理 |
| --- | --- | --- |
| `make agent-sync-check` 失败 | capability manifest、生成模板、版本控制的 `agent/mcp/tools/*.json`、`agent/mcp/openapi-facet.json`、`agent/skills` 是否不一致。 | 运行 `make agent-sync`，审核生成 diff，再运行 `make agent-sync-check`。 |
...
| `tools/list` 缺少 `orag_check` 或诊断/自运维工具 | 运行目录不在仓库根目录，或隐藏安装目录 `.mcp/tools/orag-self-*.json` 未由安装目标生成。 | 先确认版本控制的 `agent/mcp`/`agent/skills` 已通过静态检查；如需刷新隐藏安装目录，运行 `make install-agent`。 |
