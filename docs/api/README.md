...
对带受支持运行时声明的 `text-rag` Quick Pack，`POST /v1/projects/{project_id}/tutorial-experiments/{experiment_id}/runs` 接收 `variant` 与幂等键；未传 `variant` 等价于 P0 `baseline`。服务端从保存的校验后 Manifest 推导私有对象、知识库、数据集、`realtime` profile 和 Top-K，随后创建普通评测运行。Pack 明确声明 `p1_structured_json` 或 `p2_recursive_400_80` 时，候选只能在输入 fingerprint 相同的完成 P0 之后启动，并使用独立知识库；P2 固定为 Basic parser 的 400/80 分块。`GET` 同一路径下的 `{run_id}` 返回脱敏进度、`evaluation_run_id` 和只读索引事实；`GET .../{run_id}/comparison` 只在持久化证据可证明可比性时返回真实评测指标和 `index_metrics`，`:cancel` 请求取消运行。P3–P8、Replay，以及视觉/视频运行仍不属于本阶段。完整前置条件与恢复方式见 [`../tutorials/clone-and-pack-install.md`](../tutorials/clone-and-pack-install.md)、[`../tutorials/p1-structured-json-candidate.md`](../tutorials/p1-structured-json-candidate.md) 和 [`../tutorials/p2-recursive-chunk-candidate.md`](../tutorials/p2-recursive-chunk-candidate.md)。
