...
describe('API Debugger', () => {
  it('runs a query and reveals its trace', async () => {
    server.use(
      ...
      http.get('/v1/traces/trace_debug', () => HttpResponse.json({ trace_id: 'trace_debug', tenant_id: 'tenant_a', project_id: 'prj_a', pipeline_version_id: 'pv_frozen', release_id: 'rel_production', environment: 'production', profile: 'realtime', latency_ms: 42, created_at: '2026-07-11T00:00:00Z', has_error: false, error_count: 0, node_spans: [{ id: 'span1', node_name: 'retrieve', sequence: 1, latency_ms: 12, started_at: '', ended_at: '', created_at: '' }] })),
    )
    ...
    await userEvent.selectOptions(knowledgeBase, 'kb_a')
    ...
    const retrieve = await screen.findByText(/retrieve/)
    expect(retrieve).toBeVisible()
    expect(retrieve.closest('.trace-detail')).toHaveTextContent('1 个节点')
    expect(screen.getByText('pv_frozen')).toBeVisible()
    expect(screen.getByText('rel_production')).toBeVisible()
