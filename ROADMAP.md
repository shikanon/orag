...
最新进展：Pack 声明的 P2 `p2_recursive_400_80` 与 P3 `p3_contextual_retrieval` 均为 P0 的直接子实验。P3 保持 P0 的 Basic/800/120 形状，只启用固定提示词、固定上限和严格失败的服务器上下文化；运行持久化实际上下文化 chunk 数与平均上下文文本单元，比较将这些 `index_metrics` 与普通评测指标严格分开。受控 `1.0.3` fixture 与真实 PostgreSQL/Qdrant/浏览器 E2E 证明 P0→P3 的隔离和可测上下文化事实；官方公开 `1.0.3` Pack 仍需独立匿名 HTTPS、MIME、长度和 SHA-256 发布流水线。
...
- 已完成官方教程的 clone 与 Pack 安装、声明运行时的 `text-rag` Quick Pack 固定 P0 基线，以及 Pack 声明的 P1 结构化 JSON、P2 400/80 递归分块与 P3 严格失败上下文化候选（冻结输入、独立索引、真实评测指标和测量到的索引事实对比）。待完成 P4–P8 候选/Benchmark Run、官方 Replay 和视觉/视频执行；默认公共 OSS 可用性及官方 `1.0.1`–`1.0.3` Pack 发布仍是外部前置条件。
