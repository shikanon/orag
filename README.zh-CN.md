...
<p>
  <a href="./README.zh-CN.md"><img alt="README in Simplified Chinese" src="https://img.shields.io/badge/简体中文-DBEDFA"></a>
  <a href="./README.md"><img alt="README in English" src="https://img.shields.io/badge/English-F5F5F5"></a>
...
ORAG 使用 `experimental`、`beta`、`stable` 标注每项公共能力的兼容性承诺。每个 HTTP operation 都在 OpenAPI 中通过 `x-orag-maturity` 单独标注；`v0.1.0-beta.3` 发行版自身的 Beta 标签不表示其中每项能力都已达到 `beta`。在真实项目中使用 experimental 能力前，需要独立验证和明确回退方案。

`v0.1.0-beta.1` 是 ORAG 的首个公开 Beta 发行版；`v0.1.0-beta.3` 是当前推荐的发行版本。
...
flowchart TB
  ...
  rag_graph --> ingest
  rag_graph --> fusion
  rag_graph --> providers["模型 Provider<br/>Ark / 豆包 · Embedding · Rerank · 多模态"]
  fusion --> providers
  providers --> response["回答 · 引用 · Trace · Metrics"]
  rag_graph --> evaluation["数据集 · 评估 · Optimizer"]
  evaluation --> postgres
...
`TUTORIAL_CATALOG_BASE_URL` 默认是 `https://lensrhyme.tos-cn-hongkong.volces.com/tutorial-packs`。它只用于把内嵌模板中的相对 Manifest 或 Protocol 路径解析为公开下载 URL，不读取 AK/SK，也不承载用户私有数据。`text-rag/1.1.0` 已公开发布：Quick、Benchmark、完整上游数据归档、来源元数据与 `SHA256SUMS` 都支持匿名 HTTPS 读取和校验；详见 [`docs/tutorials/text-rag-pack-release.md`](./docs/tutorials/text-rag-pack-release.md)。`visual-document-rag/1.0.1` 则以不镜像上游数据的 ViDoSeek Recipe 发布；Quick/Benchmark Recipe 与校验清单位于 [`tutorial-recipes/`](./tutorial-recipes/visual-document-rag/1.0.1/)，并使用显式 guarded publish/verify 命令。详见 [`docs/tutorials/visual-document-rag.md`](./docs/tutorials/visual-document-rag.md)。`video-rag/1.0.0` 发布的是不含 Video-MME 媒体、标注、字幕、问答或派生证据的私有导入 Protocol；详见 [`docs/tutorials/video-rag-private-benchmark.md`](./docs/tutorials/video-rag-private-benchmark.md)。已安装且声明运行时的 Text Quick Pack 支持 server-owned P0 baseline；P1/P2/P3 使用独立候选索引，`p4_sparse_retrieval`、`p5_multi_query_retrieval`、`p6_rerank_retrieval` 与 `p8_context_pack` 复用兼容 P0 索引，分别只切换 pure sparse、三路 multi-query、rerank 和 Context Pack 的证据条数；`p7_graph_retrieval` 创建独立图索引并仅启用图扩展检索。tutorial evaluator v5 显式使用 hybrid P0–P6 基础检索器、固定 P0–P7 的 Context Pack 为 5/6000，并关闭图/RAPTOR 入库、rerank（P6 除外）、rewrite、HyDE、缓存与 Pipeline，以防默认运行时开关污染消融。比较返回持久化的标准评测指标与实际索引事实，不虚构质量、关系数量、成本或延迟。
