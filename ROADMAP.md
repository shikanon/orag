...
当前进展：教程实验室已支持持久化、幂等、可恢复的模板 clone 与 Pack 安装，以及 `text-rag` Quick/Benchmark 的 P0–P8 单变量运行。受控 Benchmark Pack 固定 `high_precision`/Top-K 8，持久化 Manifest SHA-256、运行环境 SHA-256、构建版本、评测输入和 evaluator v5 指纹；比较只接受直接 P0 血缘和完全一致的复现证据。受控 fixture 覆盖真实 PostgreSQL + Qdrant/浏览器路径；官方 Pack 仍需通过独立发布流水线以匿名 HTTPS 和 SHA-256 校验发布。默认公共 OSS 在 2026-07-16 仍返回 `403 AccessDenied`，因此生产可用性仍取决于对象和匿名读 ACL 发布。官方 Replay，以及视觉/视频 Live Run 仍待完成。
...
- 已完成官方教程的 clone 与 Pack 安装、Quick P0–P8 候选，以及受控 `text-rag` Benchmark Run（冻结 `high_precision`/Top-K 8 输入、真实评测指标、P0→P8 复现、Manifest/环境 SHA-256 与构建版本审计）。待完成官方 Replay 和视觉/视频执行；默认公共 OSS 可用性及官方 Pack 发布仍是外部前置条件。
