...
EVAL_QUERY="${EVAL_QUERY:-Which vector database does ORAG use?}"
GROUND_TRUTH="${GROUND_TRUTH:-Qdrant}"
PROFILE="${PROFILE:-realtime}"
TOP_K="${TOP_K:-8}"

dataset_response="$(request_json POST /v1/datasets "{\"name\":\"$(json_escape "$DATASET_NAME")\",\"kind\":\"$(json_escape "$DATASET_KIND")\"}" "$token")"

dataset_id="$(printf '%s\n' "$dataset_response" | extract_json_string id)"
if [ "$dataset_id" = "" ]; then
  printf '%s\n' "$dataset_response" >&2
  die "dataset response did not contain id"
fi
printf '%s\n' "$dataset_id" > "$DATASET_ID_FILE"
info "saved dataset id to $DATASET_ID_FILE"
