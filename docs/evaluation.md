...
| 指标 | 定义 | 适用条件与缺失标注行为 |
| --- | --- | --- |
| `pairwise_accuracy` | 当前优化器主质量指标。现阶段没有接入 pairwise judge 时，它由 deterministic `accuracy` 填充；后续接入 A/B judge 后应表示候选回答在成对比较中胜出或不输基线的比例。 | 所有运行都会返回。当前规则实现下可直接用于排序，但语义是“主指标占位 + 兼容规则命中率”，不是人工或 LLM 成对裁决。 |
| `accuracy` | 逐样本规则命中为 1，否则为 0；命中条件是答案包含 `ground_truth` 中长度大于 3 的关键项，或响应中存在任意引用。运行级为命中样本数除以样本总数。 | 兼容字段。`ground_truth` 为空且没有 citation 时通常为 0；存在 citation 仍会命中，因此不能单独证明答案正确。 |
| `hit_rate` | 当前与 `accuracy` 保持一致，都是同一组规则命中的聚合结果。 | 兼容字段，推荐新排序逻辑读取 `pairwise_accuracy`。 |
| `context_recall` | 当 `relevant_doc_ids` 非空时，统计 retrieved chunks 覆盖了多少不同相关文档 ID。 | 未标注 `relevant_doc_ids` 时退化为“是否有任意 retrieved chunk”：有召回为 1，无召回为 0。 |
| `citation_precision` | 当响应有引用且 `relevant_doc_ids` 非空时，统计引用文档 ID 落在相关文档列表中的比例。 | 没有 citation 时为 0；未标注 `relevant_doc_ids` 且存在 citation 时为 1。 |
| `ndcg_at_k` | 基于相关文档在前 `top_k` 召回结果中的排名计算归一化 DCG。越靠前命中相关文档分数越高。 | 依赖 `relevant_doc_ids`；未标注时返回 0。`top_k <= 0` 时使用实际召回结果长度。 |
| `recall_at_k` | 前 `top_k` 召回结果覆盖相关文档的比例。 | 依赖 `relevant_doc_ids`；未标注时返回 0。重复命中同一相关文档只计一次。 |
| `mrr` | 第一个相关文档的 reciprocal rank。 | 依赖 `relevant_doc_ids`；未标注或无相关召回时为 0。 |
| `map` | 对相关文档命中位置的 precision 做平均，再按相关文档数归一。 | 依赖 `relevant_doc_ids`；未标注或无相关召回时为 0。 |
| `coverage` | 样本是否至少召回一个相关文档，运行级为样本平均值。 | 依赖 `relevant_doc_ids`；未标注时为 0。 |
| `retrieval_failure_rate` | 样本标注了相关文档但没有召回任何相关文档时为 1。 | 依赖 `relevant_doc_ids`；未标注时为 0，避免把未标注样本误判为失败。 |
| `redundancy_rate` | 重复 chunk 数除以召回结果数；重复判定优先使用 chunk ID，其次 metadata 中的 hash/dedupe key，再退化到规范化文本。 | 不依赖人工标注；无召回结果时为 0。 |
| `duplicate_count` | 召回结果中的重复条数。 | 不依赖人工标注；无召回结果时为 0。 |
| `deduped_top_k_count` | 去重后的召回结果数量。 | 不依赖人工标注；无召回结果时为 0。 |
| `alpha_ndcg` | 多样性敏感的 NDCG，对重复覆盖同一 aspect/subquestion 的增益做衰减。 | 依赖 `diversity_annotations`；缺失有效 aspect/subquestion 标注时不写入逐样本指标，也不会参与运行级聚合。 |
| `aspect_coverage` | 已召回证据覆盖的 aspect/subquestion 数占全部标注 aspect/subquestion 数的比例。 | 依赖 `diversity_annotations`；缺失有效标注时跳过。 |
| `latency_p95_ms` | 本次评估内所有样本查询延迟的 P95，来自 RAG 响应的 `LatencyMS`。 | 不依赖标注。样本数很少时 P95 更接近单次或少量请求的尾部观测，需要结合多次运行判断。 |
| `cache_hit_rate` | 本次评估中 `CacheStatus == "hit"` 的样本比例。 | 不依赖标注。语义缓存配置或命中状态异常会影响解释。 |

适用边界：

- 适合做基础回归门禁、不同 profile / `top_k` 的粗粒度对比，以及检索排序、覆盖率、冗余度、多样性、引用、延迟和 cache 行为的冒烟检查。
- `pairwise_accuracy` 是当前优化器主指标；在真正的 pairwise judge 接入前，它仍由规则命中率填充，因此只能代表当前 deterministic 质量信号。
...
`POST /v1/optimizations` 对候选 `profiles` 和 `top_ks` 做确定性网格枚举。当前优化目标是运行级 `pairwise_accuracy`；在未接入真实 pairwise judge 前，该字段由上述规则命中率填充。
...
4. 候选结果记录 `profile`、`top_k`、`score`、`pairwise_accuracy`、检索诊断字段、`latency_p95_ms` 和 `run_id`，其中 `score = metrics.pairwise_accuracy`。
...
- 分数依赖当前 `pairwise_accuracy` 主指标；在当前 rule-based 填充阶段，不能等价为真实业务满意度或完整答案质量。
