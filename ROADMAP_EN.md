...
- Clone and Pack installation, a fixed P0 baseline Live Run for runtime-declared `text-rag` Quick Packs, and the Pack-declared P1 structured-JSON and P2 400/80 recursive-chunking candidates are complete with frozen inputs, independent indexes, actual evaluation metrics, and measured index-fact comparison. P3–P8 candidate/Benchmark Runs, official Replay, and visual-document/video execution remain pending; default public OSS availability and official `1.0.1`/`1.0.2` Pack publication remain external prerequisites.
