...
Generated from `x-orag-agent-capabilities` version `1` for Codex.
...
## Codex Usage
- Read the task/spec file before invoking the API.
- Use `curl` or an equivalent HTTP client against `${ORAG_API_BASE_URL}/v1/ralph-loop`.
- Return the verdict, summary, artifacts, and `trace_id` as evidence.
