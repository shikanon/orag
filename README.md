...
ORAG 使用 `experimental`、`beta`、`stable` 标注每项公共能力的兼容性承诺。每个 HTTP operation 都在 OpenAPI 中通过 `x-orag-maturity` 单独标注；`v0.1.0-beta.1` 发行版自身的 Beta 标签不表示其中每项能力都已达到 `beta`。在真实项目中使用 experimental 能力前，需要独立验证和明确回退方案。
