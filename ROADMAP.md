...
日期是目标窗口，不替代质量门禁。未达到退出条件时，版本和里程碑顺延。

| 里程碑 | 目标窗口 | 结果 |
| --- | --- | --- |
| M0：可信开源基线 | 2026-07 | 建立社区治理、安全入口、成熟度标注和受保护的主分支 |
| M1：`v0.1.0-beta.1` | 2026-08 | 发布可下载、可运行、可嵌入、无需真实 Key 即可体验的首个 Beta |
| M2：生产试点基线 | 2026-09 至 2026-10 | 完成一致性、安全、可观测和 CI/CD 加固，支持参考部署 |
| M3：评测优先控制面 | 2026-11 至 2027-01 | 完成编排、评测门禁、发布回滚和教程实验闭环 |
| M4：生态与 `v1.0` 准备 | 2027-02 至 2027-07 | 稳定扩展点、治理和兼容政策，积累生产与社区证据 |
...
- 建立 `CHANGELOG.md` 和公开 roadmap 更新流程。每次 minor release 更新能力矩阵；每季度审阅本文件。
...
- 维持可预测的月度 release、季度 roadmap review 和公开 changelog。
...
- 每个里程碑拆为独立、可测试、可审查的 implementation plan；路线图本身不替代工程设计。
- Maintainer 每月更新里程碑状态，每季度根据生产反馈、社区需求和维护能力调整优先级。
- 路线图变化应通过 PR 完成，并说明变化原因、影响的里程碑和指标。
