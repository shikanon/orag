...
| 学习工程模块效果 | 控制台 `/tutorials` -> [`tutorials/clone-and-pack-install.md`](./tutorials/clone-and-pack-install.md) -> [`tutorials/text-rag-benchmark-run.md`](./tutorials/text-rag-benchmark-run.md) -> [`tutorials/text-rag-official-replay.md`](./tutorials/text-rag-official-replay.md) | `text-rag` Quick/Benchmark 可在冻结输入下运行 P0–P8；受控 Benchmark Run 持久化复现证据。官方 text-rag Replay 是固定环境的只读快照；视觉/视频 Replay 与 Live Run 仍未开放。 |
