...
| 应用内 trace、Prometheus 指标、ready/health | `beta` | 完成指标持久化；按试点容量校准 Collector 留存与 tail sampling |
...
- 已接入可选 OpenTelemetry trace/metrics exporter，并提供可导入的 Prometheus/Grafana 资源、基础告警规则及可选 pilot Compose 覆盖层（持久 Prometheus、显式参考留存和 policy-only Collector tail sampling）；OTLP trace 支持 W3C `traceparent` 跨服务传播和 parent-based ratio head sampling。实际试点容量的指标留存、受控 trace backend 与 tail-sampling 校准仍由 operator 完成。
