...
ORAG 使用 `experimental`、`beta`、`stable` 标注每项公共能力的兼容性承诺。当前能力均为 `experimental`，在真实项目中使用前需要独立验证和明确回退方案。计划中的 `v0.1.0-beta.1` 是首个 Beta **发行版**，不表示其中每项能力都已达到 `beta`。ORAG 在 `v1.0.0` 前不会将任何能力标记为 `stable`。
