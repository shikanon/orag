...
受控试点可使用 [`deployments/docker-compose.observability.yml`](../../deployments/docker-compose.observability.yml) 覆盖层持久化 Prometheus 指标并加载现有告警规则。它的 7 天/10 GB 留存与 10% sampling 只是可审查起点；Collector 默认使用 `nop` exporter，必须在实际容量、保留期和访问控制评审后替换为受控 trace backend，不能据此宣称生产留存已完成。
