...
compose=(docker compose -p orag-tutorial-benchmark-e2e -f deployments/docker-compose.test.yml)
database_url="postgres://orag:orag@127.0.0.1:55432/orag_tutorial_clone_e2e?sslmode=disable"
tmp="$root/.tmp/console-real-tutorial-benchmark-e2e"
...
export JWT_SECRET=orag-tutorial-benchmark-e2e-jwt-secret
export API_KEY_PEPPER=orag-tutorial-benchmark-e2e-api-key-pepper
...
export PORT=18083
export ORAG_CONSOLE_API_TARGET=http://127.0.0.1:18083
export ORAG_REAL_TUTORIAL_BENCHMARK_E2E=1
export ORAG_BUILD_REVISION=benchmark-e2e-v1
export ORAG_TEST_MODE=true
export TUTORIAL_CATALOG_BASE_URL=http://127.0.0.1:18084
...
# The embedded production catalog intentionally remains on the published 1.0.0
# Pack. This controlled test maps only the immutable 1.0.9 Benchmark fixture
# to its temporary benchmark path; no source fixture or public Pack is changed.
cp -R "$root/tests/fixtures/tutorial-packs" "$public_catalog"
if test -e "$public_catalog/text-rag/1.0.0/benchmark"; then
  find "$public_catalog/text-rag/1.0.0/benchmark" -depth -delete
fi
cp -R "$root/tests/fixtures/tutorial-packs/text-rag/1.0.9/benchmark" "$public_catalog/text-rag/1.0.0/benchmark"
sed -i.bak 's/"version": "1.0.9"/"version": "1.0.0"/' "$public_catalog/text-rag/1.0.0/benchmark/manifest.json"
unlink "$public_catalog/text-rag/1.0.0/benchmark/manifest.json.bak"
"${compose[@]}" up -d --wait
...
python3 -m http.server 18084 --bind 127.0.0.1 --directory "$public_catalog" >"$fixture_log" 2>&1 &
fixture_pid=$!
WAIT_READY_ATTEMPTS=60 ./scripts/wait-ready.sh http://127.0.0.1:18084/text-rag/1.0.0/benchmark/manifest.json

go run ./cmd/oragctl migrate
go build -o "$tmp/orag-api" ./cmd/orag-api
"$tmp/orag-api" >"$api_log" 2>&1 &
api_pid=$!
WAIT_READY_ATTEMPTS=120 ./scripts/wait-ready.sh http://127.0.0.1:18083/readyz

npm --prefix console run test:e2e -- e2e/real-backend-tutorial-benchmark.spec.ts
