...
func TestMetricRegistryDrivesAggregationWhitelist(t *testing.T) {
	...
	if shouldAggregateItemMetric("latency_ms") {
		t.Fatal("latency_ms should not aggregate into an average metric")
	}
	if shouldAggregateItemMetric("faithfulness") {
		t.Fatal("faithfulness should not aggregate until judge aggregation semantics are implemented")
	}
